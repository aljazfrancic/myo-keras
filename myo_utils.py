...
GROKKING_EPOCHS = 500_000
GROKKING_TRAIN_SUBSET = 100
# Sweep run count = len(GROKKING_ARCHITECTURES) * len(GROKKING_LRS) * len(GROKKING_WEIGHT_DECAYS).
GROKKING_WEIGHT_DECAYS = [0.01, 0.1, 0.5]
GROKKING_ARCHITECTURES = [[512, 256, 128], [200, 100, 70]]
GROKKING_LRS = [0.0001, 0.0003]
# Same init for every sweep run (isolates init from hyperparams; training may still be non-deterministic).
GROKKING_INIT_SEED = 42
GROKKING_VAL_SUBSET = 8_000
GROKKING_LOG_EVERY = 1000
